...
public class Prelude implements ArendPrelude {
    ...
    public static final ModuleLocation MODULE_LOCATION =
            new ModuleLocation(LIBRARY_NAME, true,
                    ModuleLocation.LocationKind.GENERATED, MODULE_PATH);
    ...
    public static void update(Definition definition) {
        switch (definition.getReferable().textRepresentation()) {
            ...
            case "Nat":
                ...
                DIV_MOD_TYPE = new SigmaExpression(Sort.SET0, parameter(true,
                        Arrays.asList(null, null), Nat()));
                break;
            case "Fin":
                ...
                if (FIN.getConstructors().isEmpty()) {
                    FIN_ZERO =
                            new Constructor(new LocatedReferableImpl(Precedence.DEFAULT, "zero", FIN.getRef(), GlobalReferable.Kind.CONSTRUCTOR), FIN);
                    DependentLink binding = new TypedDependentLink(true, "n",
                            ExpressionFactory.Nat(),
                            EmptyDependentLink.getInstance());
                    List<ExpressionPattern> patterns =
                            Collections.singletonList(new ConstructorExpressionPattern(new ConCallExpression(SUC, Levels.EMPTY, Collections.emptyList(), Collections.emptyList()), Collections.singletonList(new BindingPattern(binding))));
                    ...
                    FIN_SUC =
                            new Constructor(new LocatedReferableImpl(Precedence.DEFAULT, "suc", FIN.getRef(), GlobalReferable.Kind.CONSTRUCTOR), FIN);
                    FIN_SUC.setPatterns(patterns);
                    FIN_SUC.setParameters(new TypedDependentLink(true, null,
                            new DataCallExpression(FIN, Levels.EMPTY,
                                    new SingletonList<>(new ReferenceExpression(binding))), EmptyDependentLink.getInstance()));
            ...
            case "Path":
                ...
                PATH.setSort(new Sort(new Level(LevelVariable.PVAR),
                        new Level(LevelVariable.HVAR, -1)));
            ...
            case "=": {
                ...
                DataCallExpression dataCall =
                        (DataCallExpression) PATH_INFIX.getBody();
            ...
            case "idp": {
                ...
                Sort sort = new Sort(new Level(LevelVariable.PVAR),
                        Level.INFINITY);
                IDP.setBody(new PathExpression(pathExpr.getLevels(),
                        new LamExpression(sort,
                                UnusedIntervalDependentLink.INSTANCE,
                                args.get(0)), new LamExpression(sort,
                        UnusedIntervalDependentLink.INSTANCE,
                        ((LamExpression) pathExpr.getArgument()).getBody())));
                break;
            }
            case "@": {
            ...
            case "!!":
                ...
                break;
            default:
                throw new IllegalStateException("In Prelude initialize(), " +
                        "unexpected symbol: " +
                        definition.getReferable().textRepresentation());
        }
    }
